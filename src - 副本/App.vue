<template>
  <div class="my-app">
    ...
    <div class="view-box">
      ...
      />
      <!-- 头部 end -->
      <div :class="`router-view-box ${showHead ? 'app-head' : ''} ${showBar ? 'app-bar' : ''}`">
        <transition :name="'vux-pop-' + (direction === 'forward' ? 'in' : 'out')">
          <router-view class="router-view" ref="myRouter" :style="`${clientInfo.routerHeight}`"/>
        </transition>
      </div>
      <!-- 底部导航 start -->
...
export default {
  ...
  watch: {
    path (path) {
      this.setLayout()
      this.initChangePath() // 当跳转页面时初始化值
    },
...
<style lang="less">
  *{
  ...
  @font-face {
    ...
    src: url('//at.alicdn.com/t/font_2042319_x52ra60mu7i.eot');
    src: url('//at.alicdn.com/t/font_2042319_x52ra60mu7i.eot?#iefix') format('embedded-opentype'),
    url('//at.alicdn.com/t/font_2042319_x52ra60mu7i.woff2') format('woff2'),
    url('//at.alicdn.com/t/font_2042319_x52ra60mu7i.woff') format('woff'),
    url('//at.alicdn.com/t/font_2042319_x52ra60mu7i.ttf') format('truetype'),
    url('//at.alicdn.com/t/font_2042319_x52ra60mu7i.svg#iconVisitor') format('svg');
  }
  .icon-visitor {
