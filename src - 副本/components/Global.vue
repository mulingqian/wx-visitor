...
const CFG = {
  ...
  productName: window.Global_CONFIG.productName !== undefined && window.Global_CONFIG.productName !== null ? window.Global_CONFIG.productName : '来访系统',
  ...
  jsAPIs: ['getLocation', 'openLocation', 'closeWindow', 'scanQRCode'],
...
const qdglobal = {
  ...
  async getData (url, ps) {
    ...
    var response = await axios.post(`${CFG.APIHOST}${url}`, ps).catch((error) => {
      console.log(error)
    })
